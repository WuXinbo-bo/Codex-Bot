...
- **轻量提醒**：开始任务短暂提示；完成条持续等待确认，展开任务面板、拖拽和查看任务都不会清除。需要处理与失败提醒保留到状态解除或明确确认。
- **可交互桌面伙伴**：注视、点击、长按、拖拽、碰边、任务板交互；白色紧凑面板随机器人移动。
- **丰富但不抢戏**：257 个表情与姿态，包含 24 个情绪家族的 96 套基础表达与 12 套复合小心思；另有 435 套活动（含 48 套约 15 秒连续动作和 70 套独立任务表演）、15 套眼部方案、18 种形状、45 件道具、24 种整脸面具。
- **日常小心思**：基础神态保持约 6–12 秒，眼神、轻点头和手部动作自然错开；18 种无嘴巴情绪装饰按情境出现，同一种情绪轮换不同表达。
- **有记忆的小故事**：5 条故事线、9 段新编动作，排练后递交、藏起再拿出道具、完成后等待回应。
- **自动外观**：4 种整体画风、眼睛和情绪形态自然轮换，也可以单独固定；18 种基础身体轮廓统一圆润处理。
- **会使用的道具**：翻开文件夹、收放线轴、转动风车、接回悠悠球；配件按需加载，按实际可见时长调节出场机会，任务提醒优先。
- **按场景调度**：工作与空闲使用不同动作池，结合家族轮换和道具曝光减少重复；鼠标靠近不打断连续动作，点击后可接续，任务通知与拖拽优先。自主活动有时间预算，给安静陪伴留出空间。
- **自定义**：8 套皮肤，默认柠檬黄，可开启随机换色；独立预览、换一套、固定当前，点击应用后生效。安静、自然、活泼统一为陪伴模式，另有减少动态选项。
- **原生轻量**：Tauri 2 + 系统 WebView2，不把 Electron 或 Node.js 打进正式安装包。

## 表情与动作

48 套连续编排，包含 44 套自主小剧场和 4 套情境接续；配合专注思考、好奇张望、犹豫检查、疲惫伸展，随着任务和你的操作切换神态。

![24 种代表性神态：专注、好奇、思考、惊讶、犹豫、疲惫与放松](docs/assets/expressions.png)

| 内容 | 当前规模 | 例子 |
| --- | --- | --- |
| 表情与姿态 | 257 个 | 108 套基础情绪表达 + 149 个保留的动作与神态姿势 |
| 情绪装饰 | 18 种 | 红晕、水光、汗滴、迟疑、压力、暖色与短符号 |
| 活动编排 | 435 套 | 新增 96 套情绪活动、36 套任务表演、16 套面板交互、16 套鼠标互动、16 套连续编排 |
| 独立任务表演 | 上述活动中的 70 套 | 开始 13、进行中 15、完成 17、待处理 10、失败 9、停止 6 |
| 眼部方案 | 15 套 | 8 套常驻眼型、7 套情境眼神；统一眨眼、视线与眼皮裁切 |
| 连续小剧场与接续 | 上述活动中的 48 套 | 每套约 15 秒，带准备、表演和收尾；接续需要对应情境，不会无条件播放 |
| 整脸面具 | 24 种 | 配合 6 种佩戴动作，作为完整面具出场 |
| 道具与配件 | 45 件 | 放大镜、笔记本、文件夹、托盘、拼图、盆栽 |
