...
test('all twenty new prop routines are reachable and end with clean hands',()=>{
  ...
  for(const prop of ['notebook','pencil','hourglass','stamp','flag']){
    ...
    const clips=Object.entries(A.CLIPS).filter(([id,frames])=>!A.Scores.meta[id]&&!A.THEATERS[id]&&!A.NARRATIVES[id]&&!id.startsWith('performance_')&&frames.some(f=>f.pose.accessories?.[prop]));
