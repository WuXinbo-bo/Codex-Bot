async(page)=>{
 ...
 const counts={};for(const [id,expected] of Object.entries(inventory)){
   await page.evaluate(id=>review.showCatalog(id),id);counts[id]=0;
   do{counts[id]+=await page.locator(`#${id}>figure`).count();if(await page.locator('#catalogNext').isDisabled())break;await page.locator('#catalogNext').click();}while(true);
   if(counts[id]!==expected)throw Error(id+': '+counts[id]);
 }
