async(page)=>{
  ...
  report.perTaskRetention=true;
  await page.evaluate(()=>{
    for(const label of ['panel','ball'])panelDemo.frames[label].srcdoc=panelDemo.frames[label].srcdoc;
  });
  await page.waitForFunction(()=>panelDemo.frames.ball.contentWindow.__nativeBot?.inbox.items.size===1&&panelDemo.frames.panel.contentDocument.querySelectorAll('[data-status="completed"]').length===1);
